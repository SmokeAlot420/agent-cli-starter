...
export class SessionService {
    ...
    `);
        stmt.run(session.id, session.name ?? null, session.initialPrompt, session.projectPath, session.projectHash, session.gitBranch ?? null, JSON.stringify(session.options), JSON.stringify(session.history), 0, now.toISOString(), now.toISOString());
     ...
     */
    createWithId(id, initialPrompt, options = {}) {
    ...
    getMostRecent() {
      ...
      LIMIT 1
    `);
        const row = stmt.get(this.projectHash);
        if (!row)
            return null;
        return this.rowToSession(row);
    }
    ...
    `);
        const row = stmt.get(this.projectHash, name);
        if (!row)
            return null;
        return this.rowToSession(row);
    }
    ...
    list() {
      ...
      LIMIT 100
    `);
        const rows = stmt.all(this.projectHash);
        return rows.map(row => ({
            id: row.id,
     ...
     */
    updateInitialPrompt(id, initialPrompt) {
        const stmt = this.db.prepare(`
    ...
    getUIMessages(id) {
        ...
        const row = stmt.get(id);
        if (!row || !row.ui_messages)
            return [];
        try {
            return JSON.parse(row.ui_messages);
        }
        catch {
            return [];
        }
 ...
 */
export function getSessionService(projectPath) {
 ...
 */
export function closeAllSessionServices() {
