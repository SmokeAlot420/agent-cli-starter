 ...
 */
export function discoverCommandsInDir(dirPath, sourceType) {
    const commands = [];
    ...
    try {
        ...
        for (const file of files) {
            if (file.isFile() && extname(file.name) === '.md') {
                ...
                const subCommands = discoverCommandsInDir(join(dirPath, file.name), sourceType);
 ...
 */
function getBuiltinCommandsPath() {
    const packageDir = dirname(new URL(import.meta.url).pathname);
    // Handle Windows paths
    const normalizedPackageDir = packageDir.startsWith('/') && process.platform === 'win32'
        ? packageDir.slice(1)
 ...
 * ```
 */
export function discoverAllCommands(additionalPaths = []) {
 ...
 */
export function getCommand(name, commands) {
 ...
 */
export function expandCommandTemplate(template, args, argList) {
 ...
 */
export function executeCommand(commandName, args, commands) {
