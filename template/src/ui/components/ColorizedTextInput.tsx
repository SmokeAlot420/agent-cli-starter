...
const COLOR_MAP: Record<string, (text: string) => string> = {
  ...
  greenBright: (text: string) => chalk.greenBright(text),
 ...
 */
function hasUltrathinkPrefix(value: string): boolean {
 ...
 */
function colorizeChar(char: string, index: number, isCursor: boolean, hasPrefix: boolean): string {
  ...
  if (hasPrefix && index < 10) {
    ...
    const colorFn = COLOR_MAP[colorName] || ((t: string) => t);
...
export function ColorizedTextInput({
  value: originalValue = '',
  placeholder = '',
  focus = true,
  mask,
  ...
  onChange,
  onSubmit,
}: ColorizedTextInputProps): React.ReactElement {
