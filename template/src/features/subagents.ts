...
let subagentCache: SubagentCache | null = null;
 ...
 */
export const BUILTIN_SUBAGENTS: Record<string, Omit<Subagent, 'source' | 'sourceType'>> = {
  'general-purpose': {
 ...
 */
export function getDefaultSubagentPaths(): string[] {
  const paths: string[] = [];
 ...
 */
export function discoverSubagentsInDir(
  dirPath: string,
 ...
 */
export function discoverAllSubagents(additionalPaths: string[] = []): Subagent[] {
 ...
 */
export function getSubagent(name: string, subagents?: Subagent[]): Subagent | null {
 ...
 */
export function formatSubagentList(subagents: Subagent[]): string {
 ...
 */
export function buildSubagentContext(subagents: Subagent[]): string {
  if (subagents.length === 0) {
    return '';
  }
 ...
 */
export function resolveSubagentModel(model: SubagentModel | undefined, defaultModel: string): string {
