...
let mcpConfigCache: McpConfigCache | null = null;
 ...
 */
export const DEFAULT_MCP_SERVERS: McpServersConfig = defaultMcpServers;

/**
 * Cloud-only MCP servers (no local server required)
 */
export const CLOUD_MCP_SERVERS: McpServersConfig = cloudMcpServers;
 ...
 * ```
 */
export function loadMcpConfig(projectPath?: string): McpServersConfig | null {
 ...
 */
function expandEnvVariables(config: McpServersConfig): McpServersConfig {
  const expanded: McpServersConfig = {};
  ...
  if (config.type === 'stdio') {
    return {
      ...
      env: config.env ? Object.fromEntries(
        Object.entries(config.env).map(([k, v]) => [k, expandString(v)])
      ) : undefined
    };
  }
  ...
  if (config.type === 'http' || config.type === 'sse') {
    return {
      ...
      headers: config.headers ? Object.fromEntries(
        Object.entries(config.headers).map(([k, v]) => [k, expandString(v)])
      ) : undefined
    };
  }
...
export function mergeMcpServers(
  defaults: McpServersConfig,
  user: McpServersConfig | null
): McpServersConfig {
 ...
 * ```
 */
export function buildMcpConfig(options: McpOptions = {}): McpServersConfig {
  const {
...
): Promise<Record<string, { connected: boolean; error?: string }>> {
