...
async function parseArgs(): Promise<{ request: string; options: CLIOptions; interactive: boolean }> {
  ...
  while (i < args.length) {
    ...
    // Settings flags
      ...
      options.settingSources = args[++i]?.split(',') as SettingSource[];
 ...
 */
function buildAgentOptions(options: CLIOptions) {
  return {
    // Core
...
async function runInteractiveMode(options: CLIOptions): Promise<void> {
  ...
  if (isTTY()) {
    ...
    if (resumeSessionId) {
      ...
      delete (agentOpts as Record<string, unknown>).continue;
    }
    ...
    const onFirstMessage = (sessionId: string, prompt: string): void => {
      sessionService.updateInitialPrompt(sessionId, prompt);
    };

    const onShowSessions = (): SessionListItem[] => {
      return sessionService.list();
    };

    const onSessionSelect = (_session: SessionListItem): void => {
    ...
    const onLoadMessages = (sessionId: string): ConversationMessage[] => {
      return sessionService.getUIMessages(sessionId);
    };

    // Save messages to SQLite for persistence across restarts
    const onMessage = (sessionId: string, message: ConversationMessage): void => {
 ...
 */
async function runOneShotMode(request: string, options: CLIOptions): Promise<void> {
